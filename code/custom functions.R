...
CTGs.detect <- function(seu) {
  log1p.mat <- seu[["RNA"]]@data
  outliers  <- apply(log1p.mat, 2, function(x){quantile(x, .75) + 1.5*IQR(x)})
  CTGs      <- readRDS("/home/fuchunyang/Proj_CTAs/input/CTGs/CTGs.rds")
  CTGs      <- intersect(CTGs, rownames(log1p.mat))
  log1p.mat <- log1p.mat[CTGs, , drop = FALSE]
  sweep(log1p.mat, 2, outliers, '>')
}
...
calculate.n.genes.control.ST <- function(seu, feature, nbin = 24) {
  CreateRandomGenes <- function (seu, feature, nbin = nbin, seed = 1) {
    ## the code was inspired from `AddModuleScore {Seurat}`
    
    set.seed(seed)
    
    data.avg <- Matrix::rowMeans(x = seu[["Spatial"]]@data)
    data.avg <- data.avg[order(data.avg)]
    data.cut <- ggplot2::cut_number(x = data.avg + rnorm(n = length(data.avg))/1e+30, 
                                    n = nbin, labels = FALSE, right = FALSE)
    names(x = data.cut) <- names(x = data.avg)
  ...
  n.genes.control <- sapply(random.genesets, \(genes) colSums(seu[["Spatial"]]@data[genes, , drop = FALSE] > 0))
  n.genes.control <- apply(n.genes.control, 1, median)
